	...
	private SpotReadResponse search(List<SpotDistanceProjection> spotDistanceProjections) {
		...
		for (SpotDistanceProjection spotDistanceProjection : spotDistanceProjections) {
			TotalIndex totalIndex = switch (ActivityCategory.valueOf(spotDistanceProjection.getCategory())) {
				case FISHING ->
					fishingRepository.findFishingForecasts(spotDistanceProjection.getId(), now,
						TimePeriod.PM).orElseThrow().getTotalIndex();
				case SCUBA ->
					scubaRepository.findFishingForecasts(spotDistanceProjection.getId(), now,
						TimePeriod.PM).orElseThrow().getTotalIndex();
				case MUDFLAT -> mudflatRepository.findBySpotIdAndForecastDate(spotDistanceProjection.getId(), now)
					.orElseThrow()
					.getTotalIndex();
				case SURFING ->
					surfingRepository.findFishingForecasts(spotDistanceProjection.getId(), now,
						TimePeriod.PM).orElseThrow().getTotalIndex();
			};
	...
	private  List<Object> getActivityDetail(OutdoorSpot outdoorSpot, LocalDate startDate, LocalDate endDate) {
		...
		for (LocalDate date = startDate; !date.isAfter(endDate); date = date.plusDays(1)) {
			if (outdoorSpot.getCategory() == ActivityCategory.FISHING) {
				Fishing fishing = fishingRepository.findFishingForecasts(outdoorSpot.getId(), date,
					TimePeriod.PM).orElseThrow();
				...
				Scuba scuba = scubaRepository.findFishingForecasts(outdoorSpot.getId(), date,
					TimePeriod.PM).orElseThrow();
				...
				Surfing surfing = surfingRepository.findFishingForecasts(outdoorSpot.getId(), date,
					TimePeriod.PM).orElseThrow();
